...
lr_decay = 0.1              # Learning rate decay multiplier
image_size = 256
...
parser.add_argument('--train', default='/datases/train/', dest='train', required=False, help='the path of training dataset')
parser.add_argument('--val', default='/datases/test/', dest='val', required=False, help='the path of validation dataset')
...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        optimizer.step()
        if (global_step % cal_step) == 0:   #cal_step = 40
            if mse_loss.item() > 0:
        ...
        if (global_step % print_freq) == 0:     #print_freq = 100
...
def testKodak(step):
    with torch.no_grad():
        ...
        with open(log_filename, 'a') as log_file:
            log_file.write("Dataset Average result---Bpp:{:.6f}, PSNR:{:.6f}, MS-SSIM:{:.6f}, MS-SSIM-DB:{:.6f}\n".format(sumBpp, sumPsnr, sumMsssim, sumMsssimDB))
        if tb_logger !=None:
...
if __name__ == "__main__":
    ...
    tb_logger = None
    save_path = os.path.join('checkpoints', 'checkpoint', args.name)
