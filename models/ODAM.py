...
class ResBlock(nn.Module):
    def __init__(self, num_filters=128):
        ...
        self.conv1 = ODConv2d(num_filters, num_filters // 2, kernel_size=1, stride=1)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = ODConv2d(num_filters // 2, num_filters // 2, kernel_size=3, stride=1, padding=1)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv3 = ODConv2d(num_filters // 2, num_filters, kernel_size=1, stride=1)
...
if __name__ == "__main__":
    ODAM = ODAM(num_filters=128)
    
    input_tensor = torch.randn(8, 128, 64, 64)  # batch_size=8, channels=128, height=64, width=64
    
    output = ODAM(input_tensor)
